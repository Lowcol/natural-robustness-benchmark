...
class Args:
    ...
    natural_video_folder: str = "video-noise"
...
# --- NEW: RAD Augmentation Function ---
def rad_random_crop(imgs, size=84):
    """
    Vectorized Random Crop (RAD) [Laskin et al. 2020]
    Input: Batch of images (B, C, H, W)
    Output: Cropped images (B, C, size, size)
    """
    n, c, h, w = imgs.shape
    # Pad by 4 pixels on each side (standard for 84x84 -> 92x92)
    # padding tuple is (left, right, top, bottom)
    padded = F.pad(imgs, (4, 4, 4, 4), mode='replicate')
    
    # Randomly pick a top-left corner (h1, w1)
    # We use a single shared crop for the whole batch for efficiency (standard practice)
    crop_max = padded.shape[-1] - size
    w1 = np.random.randint(0, crop_max)
    h1 = np.random.randint(0, crop_max)
    
    return padded[:, :, h1:h1+size, w1:w1+size]
# --------------------------------------
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # --- APPLY RAD (Random Crop) TO BATCH ---
                # Crucial: The agent must re-crop the stored images to learn invariance
                aug_b_obs = rad_random_crop(b_obs[mb_inds])
                
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(aug_b_obs, b_actions.long()[mb_inds])
                # ----------------------------------------
        ...
        # Explicit WandB metrics logging
        if args.track:
            wandb.log({
